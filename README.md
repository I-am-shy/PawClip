...
- **面板是可拖动、可缩放的浮层**：按住顶部标题栏空白处拖走，拖四周边缘调整大小
  （380×480 ～ 760×1100）。出现在**鼠标所在那块屏**的顶部居中；一旦你自己拖过或
  缩放过，位置就归你，之后呼出不再被挪回去。尺寸会记住。
...
```bash
test/run.sh                    # 一条命令跑全部：格式 → 静态检查 → 单测 → 前端检查 → 跨平台编译
...
| Windows 只在 CI 构建，未在真机验证 | 有 `CGO_ENABLED=0` 交叉编译烟测（本机 `test/run.sh` 第 ⑤ 层 + ci.yml 的 `compile-check`）与 `windows-2022` 上的正式构建 | 能证明"编得出来"；剪贴板 / 热键 / 托盘 / 面板尺寸与缩放的真机行为未实测。面板几何里**只有纯计算部分**（顶部 12% 留白、水平居中、多显示器负原点、上下边界夹取）有单测覆盖，系统调用那一半没有 |
