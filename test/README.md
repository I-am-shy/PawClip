...
`warn` 目前只有一条：空闲常驻内存未达 §12 的 30 MB（`wails.Run` 之前的基线是
13 MB —— 超出的部分在 Wails/WebKit 侧，归因见 `docs/ACCEPTANCE.md`）。它不翻红
是刻意的：**一条永远红的判据会让人学会无视整张表**，那它就再也不报警了。
取而代之的是一条回归线——同一项超过 **90 MB** 就是 `bad`。

> 这个阈值改过一次（80 → 90）。2026-09-21 做对照实测时发现，**同一份二进制**
> 在同一台机器上会在 **60–73 MB 之间摆动**（上一版 `v0.1.0` 对照 60/64/65 MB，
> 当前版 65/66/67/73 MB——差异来自 WebKit 当刻的状态，不是版本差异）。
> 80 只比实测最大值高 9%，一次正常抖动就会假红；而回归线的用途是抓"翻倍"级
> 退化，不是抓 10%。§12 的 30 MB 目标本身**没有变**，未达仍是 `warn`。
...
- **`accept.sh` 自己把 locale 钉成 UTF-8。** ④⑥ 靠 `pbcopy` 往真剪贴板写**中文**，
  而 `pbcopy` 按 `LC_CTYPE` 解释 stdin 的字节：locale 不是 UTF-8 时它对非 ASCII
  **静默写入空字符串**——剪贴板类型还在、长度是 0，应用那边"什么都没捕获到"，
  看起来正是"捕获坏了"。2026-09-21 从自动化环境里跑就踩到了（`LC_CTYPE=C`）：
  ④⑥ 双双假红，而 ⑤ 的图片照样绿——**差异在图片走 `osascript`、绕开了 `pbcopy`**，
  这个分裂就是线索。脚本里已钉死 `LC_ALL=en_US.UTF-8`（`PAWCLIP_ACCEPT_LOCALE`
  可覆盖），并在 ④ 加了一条"我们到底写进去了没有"的前置断言，把这类环境失败
  与真正的捕获故障在输出里分开。
