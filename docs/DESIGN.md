...
- **彻底删除要一次显式确认**：归档区那条"垃圾桶"点下去之后，该条目**就地
  换成**一句问句 + `取消` / `彻底删除` 两个按钮（取消在左）。不弹浮层——
  侧栏总共 180 px，浮层没有落点；也不调原生 `confirm()`——WKWebView 里
  它需要后端实现代理才出得来。为什么值得单独一条：这是全应用唯一
  不可恢复的动作，而原来的"点一次图标变红、再点一次执行"那层约定在屏幕上
  没有任何文字说明它存在。
