...
| 11 | 打开文件管理器后出现一个"透明窗口" | 面板窗口是**非不透明**的（`setOpaque:NO` + `clearColor`，为的是让系统投影跟着圆角卡片走），它自己一个像素都不画，可见性完全依赖 WKWebView 合成内容。于是"透明窗口"只有一种成因：**窗口可见，但内容没合成**。最容易踩进去的是"把路径交给访达/资源管理器"这条路——`open` 让访达激活，本 App 随即失去活跃，而失焦自动收起（第 10 条）要等 80ms 复核才发现，`orderOut` 于是落在 **App 已不活跃**的时刻，窗口再被呼出时 WebKit 未必马上重新合成。处理：`revealAndStepBack`（dialogs.go）在交给系统**之前**先收起面板，让 `orderOut` 发生在面板仍持有键盘、App 仍活跃的时刻。排查同类问题时打开窗口状态探针（`PAWCLIP_PANEL_DIAG=1` 从终端起，见 panel_darwin.h），它每次显隐/失焦判定都打一行 `visible/key/appActive/modal/webInPanel/frame`——其中 `webInPanel` 是判据：为 0 说明 WebView 已不在面板视图树里，那是另一种成因 |
