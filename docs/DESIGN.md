...
### 0.6 破坏性动作的确认：一律自绘模态框

**不可恢复的动作执行前必须问一次，而且只能用自绘的 `ConfirmModal`
（`frontend/src/components/ConfirmModal.tsx`）——不能用 `window.confirm`。**

原生 `confirm()` 在这个项目里不是"长得难看"，而是**根本不出现**：打包产物跑
在 WKWebView 里，而 Wails v2.16 只声明了 `WKUIDelegate` 的遵循关系，一个
`runJavaScript*Panel` 方法都没实现（`wails/v2@v2.16.0/internal/frontend/desktop/darwin`
全目录搜不到 `runJavaScript`）。WebKit 对这几个 UI 代理方法的语义就是"不实现 = 不弹"，
`confirm()` 拿到的是一个被立刻回调的假值。于是 `if (!window.confirm(...)) return`
这种写法会让按钮**按下去什么都不发生**——回收站的"清空回收站"、导出页的
"撤销上次导入"在改用自绘弹窗之前都是这个状态：不是提示的长相不对，而是
那行代码之后的动作从来没被执行过。
...
| 约束 | 理由 |
|---|---|
| 弹窗开着时**整段截住键盘**（window 上的**捕获**阶段 + `stopPropagation`） | 面板在 window 上还挂着上下键（挪光标行）、回车（**直接粘贴并收起面板**）、Backspace（删除）、⌘1..9（直贴第 N 条）。弹窗开着时这些必须失效，最要命的是回车：它会把内容贴进用户当时的前台应用，而这恰好是"正在确认一个破坏性动作"时最不该发生的事 |
| Esc 只关弹窗，**不**触发外层的"返回 / 收起面板" | 捕获先于冒泡，而 App 与面板的 Esc 监听都挂在冒泡阶段 |
| **取消**按钮拿初始焦点、写在左边 | 默认动作必须是"不发生任何事"；危险动作不放第一位 |
| 点遮罩空白 = 取消，按 `mousedown` 判定 | 在弹窗里按下、拖到外面松手时 `click` 的 target 会变成遮罩，那一下不该算"点了取消" |

不 `preventDefault` 一刀切：Tab 移动焦点、回车/空格激活当前按钮都是浏览器的默认
动作，截掉它们等于把键盘用户关在弹窗外。Tab 越界由组件内的 `trapTab` 拉回来
（否则焦点会落到遮罩后面的列表按钮上——看不见，但空格/回车照样能激活它）。

**反向要求：可恢复的动作不要加确认。**"删除"（→ 回收站）与"移入归档"都是一步
可撤销的，给它们加确认只会训练用户无脑点确定，把真正需要读的那几句也变得可跳过。
但它们的**文案**必须与不可恢复的那个区分开（§4.4.6）：同一个界面里同时摆着
"删除"和"彻底删除"时，用户分不出哪个不可逆。

四个调用点：草稿归档区的彻底删除（§4.4.6）、回收站单条与批量彻底删除
（含 Backspace 那条高速通路）、清空回收站、撤销上次导入。
...
| **免抢焦点窗口** | `NSPanel` + styleMask `.nonactivatingPanel` + 覆写 `canBecomeKey` | 窗口样式加 `WS_EX_NOACTIVATE`。**注意它拿不到键盘**（这正是 mac 侧要真 NSPanel 的原因），所以 Windows 上呼出面板时会明确抢一次前台、粘贴完成后还给原窗口——见 §8 第 5 条 |
...
- **图片宽度是可选的尾部后缀**：`![说明|300](blob/…)`（像素）。
  为什么不写在 URL 上——URL 那一段会被 `rewriteMDRefs` 逐字符扫成
  **文件引用**（读到空白或 `(` `)` 为止），往上挂任何东西都会让引用集合
  与真实文件对不上，孤儿回收随后删掉的正是用户还在用的图。alt 那一段
  没有第二个读者，是这套方言里唯一能安全扩展的位置。代价是"说明文字
  恰好以 `|300` 结尾"会被读成宽度：可接受，alt 只在图片加载失败时可见，
  而宽度是用户显式拖出来的。宽度不另开 DB 列，因为它必须与正文同生共死
  ——把一段带图的正文复制到另一条草稿，宽度得跟着走。
...
**裸 URL 自动识别（参考飞书）**：敲下空格 / 回车时，把文本节点里裸写的
`https://…` / `www.…`（收尾标点剥掉、前邻字符是字母数字的不认）换成
`<a>`。识别规则是纯函数（`md.ts` 的 `findAutoLinks`，整个
`test/check-md.mjs` 96 条断言盯着），
DOM 替换在 `Drafts.tsx` 里做并按"逻辑文本偏移"恢复光标；输入法组词中的
空格是选字动作，不触发。链接浮层带**显示文本（别名）**：光标在链接里
打开就是"编辑这条链接"（预填地址与文字），选中文字打开则预填选中内容。

**程序化插入必须同步占位提示**：贴图、插链接不走 input 事件，"正文是否
为空"的开关要显式补一次，否则占位提示要等用户敲下一个键才消失——
看起来就像"图没插上"。

**图片可以拖拽缩放**：点中图片 → 右下角出现把手 → 拖动改宽度；
双击图片清掉宽度、回到"按容器宽自适应"。宽度落在 `img` 的 `width`
**属性**上（不是内联 `style`：样式是浏览器与用户代理随时会改的东西，
让它参与存储等于让"存下来的宽度"取决于渲染环境），也就是 §4.4.1 那个
`|300` 后缀。两个实现上的要害：

- 把手是编辑区的**兄弟**节点（`.draft-editwrap` 里绝对定位），**不塞进
  `contenteditable`**——那片子树会被 `htmlToMd` 逐节点读一遍（等于往存储
  格式里加东西），而且是"React 不管、我们手改 `innerHTML`"的地方，React
  的 children 变更会去 patch 一棵已经被改乱的树。代价是图片滚动 / 重排 /
  面板改宽之后，把手的位置要自己重算。
- 拖拽上限是编辑区的**可视宽度**：超过它的话 CSS 的 `max-width: 100%`
  会把图片压回来，而存下来的那个宽度用户永远看不到。

**正文里的链接可以直接点开**：单击 `<a>` 用系统默认程序打开。三条都是硬性的：

- **必须 `preventDefault` 之后交给后端**（`App.OpenURL`）。这个 WebView 就是
  应用界面本身，放它跟着链接导航一趟等于把面板换成网页、用户回不来。
  后端还会再筛一次协议（`dialogs.go` 的 `openableURL` 只放
  `http` / `https` / `mailto`）：前端那次是为了给用户一个明确结果，后端那次
  是因为前端不可信，而这里递出去的是"**系统会去执行**的东西"——macOS 的
  `open` 连一个 `.app` 都会替你启动。跨平台实现文件照旧只管"怎么调这个系统
  的程序"（`openurl_*.go`），策略留在能测的那一侧。
- **打开前先收起面板**（复用 `hideBeforeReveal`）。与"打开数据目录"同一个坑：
  浏览器被拉起来 → 本 App 失去活跃 → 非不透明面板在"App 已经不活跃"的
  时刻被 orderOut，回来就是一块透明空壳。
- **拖选文字时不打开**（选区非折叠即视为"选字"）：从链接中间往外拖是选择，
  把浏览器弹出来是灾难性的误判。
...
**读写顺序链（v0.2 修复的缺陷）**：卸载 flush 是异步的，而 Wails 绑定调用
之间**没有顺序保证**——切走视图时 `SaveDraft` 还在路上，切回来时重新挂载的
`Draft(id)` 可能**先到**，读到旧正文；之后自动保存把旧正文写回去，最后一段
编辑就真丢了。前端把所有草稿读写串到一条**模块级** promise 链上
（组件状态活不过卸载，链必须放模块里），跨"卸载 → 重挂"保住先写后读。
标题防抖同理：切草稿前必须先结算 pending 的 `RenameDraft`，否则旧草稿的
标题定时器会在选中已换到新草稿之后触发，把旧标题写到新草稿上。
...
- 编号单独存一列而不从标题反推：标题是本地化的（中文"草稿 2" /
  ...
  集体跳变）。
- 目录可**收起**。这不是交互偏好而是**布局必需**（§4.4.5）。收起状态
  落库（`ui.draftTocCollapsed`），活过重启。
- **删除按钮在目录条目上**（悬停出现），删除 = 进归档区（§4.4.6）。
  编辑区顶栏不再放第二个删除入口：同一个动作在两处出现，
  用户会去猜两处是不是不一样。
...
- 到期回收：`archived_at` 超过 `draft.archiveTtlSec`（草稿**自己的**保留期，
  默认 30 天）由 GC 硬删，统计进
  `retention.Report.DraftsPurged / DraftsPurgedChars`。
  ⚠️ 这一条改过决策：v0.2 之前是复用 `retention.trashTtlSec`（"不另开
  key"）。实践反馈是两个保留期**互不影响**才是对的——回收站里是自动
  捕获的流水（7 天够了），归档区里是用户手写的作品（预期更长），
  共用一个值会把"清空回收站"连带成"清空草稿归档"。
- **彻底删除要一次显式确认，而且是模态框**：归档区那条"垃圾桶"点下去弹一个
  居中的确认卡片，写明"删的是哪一条"（`draft.purgeBody` 带 `{name}`），
  按钮是 `取消` / `彻底删除`，取消拿初始焦点。
  ⚠️ 这一条改过两次形态：v0.2 第一版是**就地替换那条目录条目**（"侧栏只有
  180 px，浮层没落点"），真机反馈是不合格——180 px 里那句问句会被切成省略号，
  而"删的是哪一条"恰好是被切掉的那半句。改用模态框之后，宽度不再受侧栏约束。
  第二版把弹窗**抽成了共享组件**（`components/ConfirmModal.tsx`，规格见 §0.6），
  本节只保留草稿这边特有的两条：正文明写"删的是哪一条"，以及"删除"这个
  目录按钮的文案必须写**移入归档**——两个动作在同一根侧栏里，词混了用户就
  分不出哪个是不可逆的。
  ⚠️ 另：本节原先写"全应用唯一不可恢复的动作"，不准确——回收站的"彻底删除"
  与"清空回收站"同样是不可恢复的，它们的确认框现在也走同一个组件（§0.6）。
  真正成立的表述是"**草稿本里唯一**不可恢复的动作"。原来的"点一次图标变红、
  再点一次执行"那层约定在屏幕上没有任何文字说明它存在，值得单独一条记下。
- 目录条目上的删除是**归档**（可恢复，见上），所以它不要确认，但它的
  提示文案必须写"移入归档"而不是"删除"：两个动作在同一根侧栏里，词混了
  用户就分不出哪个是不可逆的。
...
`archived_at` 超过 `draft.archiveTtlSec` 的草稿被硬删（计数进
...
| 11 | 打开文件管理器后出现一个"透明窗口" | 面板窗口是**非不透明**的（`setOpaque:NO` + `clearColor`，为的是让系统投影跟着圆角卡片走），它自己一个像素都不画，可见性完全依赖 WKWebView 合成内容。于是"透明窗口"只有一种成因：**窗口可见，但内容没合成**。最容易踩进去的是"把路径交给访达/资源管理器"这条路——`open` 让访达激活，本 App 随即失去活跃，而失焦自动收起（第 10 条）要等 80ms 复核才发现，`orderOut` 于是落在 **App 已不活跃**的时刻，窗口再被呼出时 WebKit 未必马上重新合成。处理分两层：① `revealAndStepBack`（dialogs.go）在交给系统**之前**先收起面板，避开那个时序；② **构造性消除**——内容层垫一块**不透明的圆角底座**（panel_darwin.m `g_cardLayer`，圆角/裁剪从 WebView 层挪到底座层），颜色由 hide/show 时机用 `evaluateJavaScript` 从 DOM 的 body 背景色同步（整个函数 `@try` 包裹，旁路逻辑不允许带崩显隐；初值按系统外观猜）。这样"内容没合成"的最坏结果是一块底色卡片，**不再存在透明洞**；呼出时 `invalidateShadow` 防投影残影。排查手段：窗口状态探针（`PAWCLIP_PANEL_DIAG=1` 从终端起），每次显隐/失焦判定打一行 `visible/key/appActive/modal/webInPanel/frame/screens/alpha`——`webInPanel=0` 说明 WebView 已不在面板视图树里，是另一种成因；`screens` 用于多显示器（AppKit frame 是主屏**左下**原点，截图工具 `-R` 是**左上**原点）。⚠️ 取证教训：本机终端没有"屏幕录制"权限时，macOS 对截图**只合成壁纸**，`screencapture -l` 对任何 App 的窗口都报 "could not create image from window"——先拿别的 App 窗口做对照验证权限，再相信截图结论（2026-09-18 实测踩坑） |
| 12 | 对 Wails 的 `WailsWebView` 调视图方法会崩 | `WailsWebView` 声明继承 `WKWebView`，但实测对它调 `isDescendantOfView:` 运行时抛 `unrecognized selector` 并 SIGABRT（2026-09-18，SIGABRT 栈落在 dispatch 主队列块里）。所以"找 WebView 并校验它还在视图树上"这种写法不能用：**每次现找**（浅层 `subviews` 递归，成本可忽略）、调用前 `respondsToSelector:`、整体 `@try`——见 `PawClip_WKWebView` / `PawClip_SyncBackdropFromDom` |
| 13 | 备份页点「···」弹访达后，出现半透明窗口一直占位 | Wails 的 `OpenFile*Dialog` 把 `NSOpenPanel` 以 sheet 挂在**Wails 宿主窗口**上（`WailsContext.m: beginSheetModalForWindow:self.mainWindow`），而宿主窗口的 contentView 早已被面板接管掏空——sheet 挂在隐藏/掏空的父窗口上就是坏的：实测窗口列表里 sheet 以 **alpha=0** 存在（真机上表现成一块半透明占位灰幕）。修法：`panel.RunOpenDialog`（dialog_darwin.go + `paw_open_dialog`）把 sheet 挂到**面板窗口自己**身上——入口在面板里的备份页，面板此刻必然可见，sheet 压暗有内容的面板，观感与系统一致；面板不可见时退化为无父窗口的浮动模态。失焦自动收起对 `attachedSheet` 已有豁免（第 10 条），两条逻辑自然会合。非 darwin 平台回退 Wails（那里的宿主窗口是正常窗口）。验证靠窗口列表 A/B（CGWindowList 元数据不需要录屏权限）：修复前 sheet alpha=0，修复后 alpha=1 且宿主窗口不上屏 |
...
| 5 | 面板抢焦点 | 窗口样式加 `WS_EX_NOACTIVATE` + `WS_EX_TOOLWINDOW`（并摘掉 `WS_EX_APPWINDOW`）。**Wails v2 的操作路径**：启动后 `FindWindow("wailsWindow", nil)` 拿到 HWND，再用 `SetWindowLongPtr(hwnd, GWL_EXSTYLE, ...)` 改样式。Windows 侧不需要 cgo。**⚠️ 2026-09-23 修订**：这一条原来写的是"不要调用 `SetForegroundWindow`"——那只对了一半。`WS_EX_NOACTIVATE` 的窗口确实不会被点击激活，代价是它**也收不到键盘输入**，而面板的搜索框是主要输入面（§0.2 在 macOS 上能用 NSPanel 绕过这件事，Win32 没有等价能力）。所以 `panel/windows.go` 的 `Show()` 现在**明确抢一次前台**：`ShowWindow(SW_SHOWNOACTIVATE)` → `SetWindowPos(HWND_TOPMOST, NOACTIVATE)` → `SetForegroundWindow`，由 `writer.go` 在粘贴完成后把前台还给原窗口。抢不到前台时 `Show()` 返回错误，上层推一条前端提示——与 macOS 的 `paw_show()` 契约对齐（"面板看得见、敲字没反应"必须说出来，不能静默） |
...
| 11 | **面板几何**（多大 / 出现在哪 / 能不能缩放）在 Windows 与 macOS 上不一致 | 根因是两边的形态定义不同：macOS 的面板是 `panel_darwin.m` 自建的 NSPanel，Wails 的宿主窗口只是个空壳；**Windows 没有 NSPanel，Wails 的宿主窗口就是用户看到的面板**。所以 mac 上"由 NSPanel 自己负责"的几件事，在 Windows 上必须逐条落实：<br>① **缩放**。`options.App.DisableResize` 在 Windows 上必须为 `false`：Wails 据此注入 `window.wails.flags.enableResize = true`，用自带的运行时 JS 做无边框边缘缩放（边缘光标 + `resize:<edge>` 消息 → `WM_NCLBUTTONDOWN` → 系统 SC_SIZE 循环）。保持 `true` 等于把面板钉死在一个尺寸上。（macOS 侧保持 `true`，那是 M0 PoC 验过的组合，且与 NSPanel 的能力无关。）<br>② **尺寸边界**。`Min/MaxWidth/Height` 由 main.go 从 `panel.MinPanelWidth…MaxPanelHeight` **转发**进 `options.App`，Wails 的 `winc.Form` 在 `WM_GETMINMAXINFO` 里按窗口 DPI 换算成 `ptMinTrackSize/ptMaxTrackSize`。**不要**自己子类化窗口过程去实现它——Wails 已经做了，重复实现反而会和它的消息处理打架。<br>③ **副产物：`WS_MAXIMIZEBOX` 必须摘掉**。①带来一个副作用——Wails 的 `EnableMaxButton(!DisableResize)` 会把 `WS_MAXIMIZEBOX` 加回来。而我们的拖动是把 `HTCAPTION` 当标题栏用，无边框 + 可最大化的窗口"双击标题栏即最大化"：用户在标题栏上快点两下，面板会铺满整屏。所以 `panel/windows.go` 的 `applyPanelWindowStyle` 在改样式时顺手摘掉它。<br>④ **尺寸与位置**。`Attach` 把 settings 里的尺寸设上去（Wails 只知道 main.go 那对建窗占位值 560×760），首次 `Show` 时再断言一次几何并按"**鼠标所在显示器**顶部居中"定位，语义对齐 `paw_panel_recenter`（公式抽在 `panel/geometry.go` 的 `RecenterIn`，纯函数、有单测）。用户拖过或缩放过之后位置归用户，与 mac 的 `g_userPlaced` 同语义。<br>⑤ **`Size()` 在面板被显示过之前返回 `(0,0)`**。否则 Wails 的建窗占位尺寸会被 `persistPanelSize` 当成"用户拖出来的结果"写回 `settings`，把用户在 macOS 上设的尺寸**静默覆盖掉**（`settings` 两个平台共用）。<br>**验证边界**：只有 ④ 里那部分纯计算（12% 留白、水平居中、负原点显示器、上下夹取）被 `panel/geometry_test.go` 覆盖并在开发机上跑过；其余全是系统调用，与 §8 其余各条一样**只能在真机上验**（仓库没有 Windows 机器；编译层面本机 `test/run.sh` 第 ⑤ 层与 ci.yml 的 `compile-check` 同口径，都到交叉编译为止）。另外 ④ 的公式在 mac 侧是 ObjC 副本，两处必须一起改 |
...
| `ui.lastDraftId` | `0` | 上次打开的草稿（`ui.lastView` 的草稿粒度）。重进草稿本回到它而不是固定回第一条——多条草稿时固定回第一条，看起来就像"刚写的东西没了"。指向的草稿可能已被删，读取方必须校验存在性 |
| `ui.draftTocCollapsed` | `false` | 草稿本目录的收起状态。**布局必需**而非偏好（面板 560 逻辑点下目录 + 编辑器摆不下），所以要活过重启 |
...
| `draft.archiveTtlSec` | `2592000` | 归档草稿保留 30 天。与 `retention.trashTtlSec` **互不影响**（§4.4.6 的决策修订）：归档区是用户手写的作品，保留预期长于回收站里的自动捕获流水 |
...
│  │  ├─ components/   itemlist · searchbar · preview · ttl-badge · icons · contentviewer · contextmenu · confirmmodal（破坏性动作的确认框，§0.6）
...
14. 目标：前端产物 gzip 后 < 150 KB。
    - **实测**（2026-09-21，草稿本合入后 / 发 v0.1.1 前）：JS 79.7 KB + CSS 4.9 KB = **84.6 KB**，余量约 65 KB。
    - 草稿本选自研 `contenteditable`（§4.4.2）而不是引入编辑器库，正是为了不把这 65 KB 一次吃掉：TipTap 约 60–90 KB、Quill 约 43 KB（gzip），选任何一个都要在 §12 的"前端体积"这一行上重新谈判。**这条预算不是数字洁癖**——面板是"用完即销毁"的轻量窗口（§1.2），首屏解析的就是这份 JS。
