...
- **彻底删除要一次显式确认，而且是模态框**：归档区那条"垃圾桶"点下去弹一个
  ...
  按钮是 `取消` / `彻底删除`，取消拿初始焦点。
  ⚠️ 这一条改过一次形态：v0.2 第一版是**就地替换那条目录条目**（"侧栏只有
  180 px，浮层没落点"），真机反馈是不合格——180 px 里那句问句会被切成省略号，
  而"删的是哪一条"恰好是被切掉的那半句。改用模态框之后，宽度不再受侧栏约束。
  仍然不调原生 `confirm()`：WKWebView 里它需要后端实现代理才出得来。
  为什么值得单独一条：这是全应用唯一不可恢复的动作，而原来的"点一次图标
  变红、再点一次执行"那层约定在屏幕上没有任何文字说明它存在。
  配套两条：Esc 必须**在捕获阶段**截住（否则 App 那一层的"Esc = 返回"会
  连弹窗带草稿本一起关掉），点遮罩空白处 = 取消（用 mousedown 判定，
  免得"在弹窗里按下、拖到外面松手"把弹窗关掉）。
