<!-- 用法：本文件即"提示词正文"。新会话第一条消息，把「## 任务」以下的内容全选复制粘贴即可。
     ...
     ⚠️ 存档说明：这是 M1 开工时用过的提示词，项目现在已做到 M4 收尾，本文件保留下来
     是为了追溯 M1 的范围与纪律。文档后来统一迁到了 docs/，下面所有路径已相应更新；
     面向使用者的说明在根 README.md，「当前状态」以 docs/ACCEPTANCE.md 为准。 -->
...
1. `docs/README.md` —— 开发文档索引（技术栈、已锁定决策、功能分期、开工三坑、构建口径）
2. `docs/DESIGN.md` —— **唯一权威规格**（988 行）。阅读顺序：**§0 项目基调 → §16 里程碑 → §2 跨平台架构 → §4 数据模型（含 DDL）→ §5 过期与生命周期 → §7 / §8 平台实现要点 → §14 优化清单**
3. 暂时不用读：`docs/BACKUP-FORMAT.md`（M3 才用）、`poc/`（M2 做面板时读，里面的 `panel_darwin.m` 与 `probe.go` 可直接复用）

**规则**：`docs/DESIGN.md` 是唯一真源。若我的口头要求与文档冲突，以文档为准并明确指出冲突。若你判断文档有误，先说清楚再改文档，不要静默偏离。
...
- 新增第三方依赖前先说明理由与体积代价。
- **保持 `poc/` 原样不动**，不要清理或改写。
- 不要改 `docs/DESIGN.md` 的既有决策；发现文档确有错误时，改动单独成一个 `docs:` 提交，并在回复里点出来。
